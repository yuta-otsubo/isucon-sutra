...
- 初期化処理の実行
  - POST /api/initialize（42 秒でタイムアウト）
  - POST /api/owner/owners（複数回実行、20 秒でタイムアウト）
  - POST /api/chair/chairs（複数回実行、20 秒でタイムアウト）
  - POST /api/user/users（複数回実行、20 秒でタイムアウト）
...
打ち切られた場合はその負荷走行は Failed として記録されます。
